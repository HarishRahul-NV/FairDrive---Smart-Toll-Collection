...
const Tracking: React.FC = () => {
  ...
  // WebSocket connection for GPS data - FIXED VERSION
useEffect(() => {
  ...
  if (isTracking) {
    ...
    webSocket.onmessage = (event) => {
      ...
      if (data.event === "update") {
        setLiveData(prev => ({
          ...prev,
          distance: Math.round(data.total_distance_m * 100) / 100, // ✅ Round to 2 decimals
          toll: data.current_toll,
          speed: data.speed || prev.speed
        }));
      } 
      else if (data.event === "trip_complete") {
        setLiveData({
          distance: Math.round(data.final_distance_m * 100) / 100, // ✅ Round to 2 decimals
          toll: data.final_toll,
          speed: 0
        });
        setIsTracking(false);
        setConnectionError(null);
        ...
        toast({
          ...
          description: `Final distance: ${Math.round(data.final_distance_m * 100) / 100}m, Toll: ₹${data.final_toll}`,
          variant: "default",
        });
      }
      else if (data.event === "turn") {
        // ✅ COMPLETE STATE RESET for turn events
        setLiveData({
          distance: Math.round(data.total_distance_m * 100) / 100, // ✅ Round to 2 decimals
          toll: data.current_toll,
          speed: 0
        ...
        toast({
          ...
          description: `Turn detected! Final distance: ${Math.round(data.total_distance_m * 100) / 100}m, Toll: ₹${data.current_toll}`,
          variant: "default",
        });
      }
